...
DROP TABLE Employe;
DROP TABLE Fournir;
DROP TABLE Produits; --drop table supprime l'ancien table et mise a jour des nouveaux données 
...
CONSTRAINT Fournir_Produit_Fournisseur_pk Primary KEY(Fournisseur, produit) 
);
...
CONSTRAINT Manipuler_Employe_Produit_pk Primary KEY(Prod, Emp)
);
CREATE TABLE Client(
Code_C NUMBER,
Num_tel NUMBER UNIQUE,
...
CONSTRAINT Servir_Employe_Client_pk Primary KEY(Emp,Cli)
);
